...
type Failure = "none" | "readiness" | "registry" | "origin" | "worker" | "workerMissing" | "headers" | "shape" | "payout";
const start = (failure: Failure): Bun.Server<undefined> => Bun.serve({ port: 0, fetch(request) {
  const path = new URL(request.url).pathname;
  if (path.includes("worker") && failure === "workerMissing") return new Response("not found", { status: 404 });
...
describe("marketplace beta health probe", () => {
  ...
  for (const failure of ["readiness", "registry", "origin", "worker", "workerMissing", "headers", "shape", "payout"] as const) test(`rejects ${failure}`, async () => { const server = start(failure); try { expect(runProbe(config(Number(server.port)))).rejects.toThrow(); } finally { server.stop(); } });
  test("skips the production Worker check only for a direct staging origin", async () => {
    const server = start("workerMissing");
    try { expect(await runProbe({ ...config(Number(server.port)), requireWorkerRevision: false })).toEqual({ ok: true, checks: 7 }); }
    finally { server.stop(); }
  });
