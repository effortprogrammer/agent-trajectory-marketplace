const helpTexts = new Map<string, string>([
  ["marketplace seller candidate bundle --help", "Usage: trajectory marketplace seller candidate bundle --root <absolute-dir> [--print-selection] [--out <absolute-zip> [--trace <relative-path>...] [--review-cache <absolute-dir> --review-policy <bounded-id> | --selection <absolute-json>]]\n\nPreview the exact upload set with --print-selection (no writes, no network), edit the document by removing traces, then build with --selection. Private review sidecars are content-addressed under --review-cache and are never included in the candidate archive."],
